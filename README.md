...
主窗口右上角点击 **悬浮窗**。默认约 300×180，可拖动顶部移动位置，并记住上次位置；点击减号可折叠为约 44 像素高的计时条。

- 显示当前任务、本次开始时间、本次用时及累计用时，支持暂停、继续和结束保存。
- 点击 **快速新建** 展开输入，回车立即开始，也可选择 **待办**。开始新任务会暂停当前任务。
- 悬浮窗置顶，计时更新不抢焦点。点击右上角箭头返回主窗口。
- 悬浮窗开启时关闭主窗口，会隐藏主窗口并继续计时；关闭悬浮窗不会结束任务，并会恢复已隐藏的主窗口。完全退出可先关闭悬浮窗，再关闭主窗口。

开发验证：`npm run test:floating` 使用临时用户目录运行真实 Electron 窗口，验证创建、计时、暂停、结束、窗口大小和位置恢复，不读取现有任务数据。
