...
主窗口右上角点击 **悬浮窗**。窗口固定为约 530×104 的两行小条：鼠标离开时半透明，移入时恢复清晰。拖动第二行的任务名称可移动位置，并记住上次位置。

- 第一行直接输入任务，回车或点击 **开始** 即刻计时。开始新任务会暂停当前任务。
- 第二行显示当前任务、开始时间及持续时间，支持暂停、继续和结束保存。
- 悬浮窗使用高层级置顶，计时更新不抢焦点。再次点击主窗口的 **悬浮窗** 按钮可关闭。
- 最小化主窗口时悬浮窗仍可继续计时；关闭主窗口会同时关闭悬浮窗并退出软件。
